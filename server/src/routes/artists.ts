...
async function getArtistData(artistId: string) {
  ...
  const artistData: SpotifyArtist =
    (await artistDataRequest.json()) as SpotifyArtist;
  return artistData;
}
...
async function getArtistDescription(artistName: string, preferredLang: string) {
  ...
  const fetchInfo = async (lang: string) => {
    ...
    return data?.artist?.bio?.summary
      ? cleanDescription(data.artist.bio.summary)
      : null;
  };
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const description = await getArtistDescription(
      rawArtist.name,
      selectedLang,
    );
    ...
    res
      ...
      .json({
        error:
          "Não foi possível conseguir o artista do spotify ou a descrição do last.fm",
      });
  }
});
